        ...
        for plane in self.planes:
            if plane not in self.landing_order:
                ...
                for i in range(len(self.landing_order) - 1):
                    plane1_distance = self.get_distance_to_td(self.landing_order[i])
                    plane2_distance = self.get_distance_to_td(self.landing_order[i + 1])
...
class Plane:
    ...
    def move(self):
        ...
        dx = (self.s/50 / 2 ) * math.cos(rad)
        dy = (self.s/50 / 2 ) * math.sin(rad)
        self.x += dx
        self.y += dy
